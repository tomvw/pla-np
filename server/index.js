...
const CACHE_TTL = parseInt(process.env.ART_CACHE_TTL_SECONDS || String(24 * 3600), 10); // seconds
const CACHE_MAX_BYTES = parseInt(process.env.ART_CACHE_MAX_BYTES || String(200 * 1024 * 1024), 10); // default 200MB
let CACHE_HITS = 0;
let CACHE_MISSES = 0;
  ...
  try {
    ...
    for (const name of files) {
      ...
      try {
        ...
        metas.push({ meta, metaPath, dataPath, mtime: meta.timestamp || stat.mtimeMs, size: stat.size });
        total += stat.size;
      } catch (err) {
        await fs.promises.unlink(metaPath).catch(() => {});
      }
    ...
    for (const item of metas) {
      if (now - (item.meta.timestamp || 0) > CACHE_TTL * 1000) {
        await Promise.all([fs.promises.unlink(item.dataPath).catch(() => {}), fs.promises.unlink(item.metaPath).catch(() => {})]);
        total -= item.size;
      }
    }
    // enforce max size by deleting oldest
    const remaining = metas.filter(item => now - (item.meta.timestamp || 0) <= CACHE_TTL * 1000);
    if (total > CACHE_MAX_BYTES) {
      remaining.sort((a, b) => (a.meta.timestamp || 0) - (b.meta.timestamp || 0));
...
app.get('/api/art', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      keyStr = thumb;
      targetUrl = thumb.startsWith('/') ? `${config.PLEX_URL.replace(/\/$/, '')}${thumb}` : thumb;
    }
    ...
    // Try cache hit
    try {
      const metaRaw = await fs.promises.readFile(metaPath, 'utf8').catch(() => null);
      if (metaRaw) {
        ...
        if (Date.now() - (meta.timestamp || 0) <= CACHE_TTL * 1000) {
          ...
          // expired
          await Promise.all([fs.promises.unlink(dataPath).catch(() => {}), fs.promises.unlink(metaPath).catch(() => {})]);
        }
      }
    } catch (err) {
  ...
  try {
    ...
    for (const name of files) {
      ...
      try {
        ...
        top.push({ key: name.replace(/\.json$/, ''), size: stat.size, timestamp: ts });
      } catch (err) {
        // ignore
      }
    }
